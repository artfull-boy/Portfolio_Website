...
function Competence() {
  return (
    <div className='competence container'>
        ...
        {jsonfile.map((skill) => (
        <div key={skill.domain} className='relative do md:flex md:flex-row flex flex-col gap-8 justify-between px-[65px] py-[50px] w-full content-center bg-[#F5F5F5] rounded-[71px] lg:h-[480px] h-auto overflow-hidden'>
            ...
            <img src={skill.picture} className='picture md:max-w-[350px] md:block hidden z-10 object-contain'/>
        </div>
  ))}
  </div>
  )}
